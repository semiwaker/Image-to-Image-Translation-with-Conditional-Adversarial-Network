...
edgesshoes数据集的输入是各种鞋子的线图，输出是实物的图片。

![shoe1](images/shoe1.jpg)

![shoe2](images/shoe2.jpg)

我们的生成器学出了鞋子大体的形状，但是细节部分学得不是很好。输入的线图中细节并不是很足够，可能需要领域相关的知识来补充才能学出比较真实的细节。

edges2handbags数据集的输入是各种包的线图，输出是实物的照片。
...
在night2day数据集上，我们的效果比较不好。night2day数据集的输入是白天的图片，输出是同一位置夜晚的图片。由于建筑物较暗，而天空较亮，基本只学到了天空的部分。目前的辨别器对不同亮度的部分是一视同仁的，这一点有待改进。
